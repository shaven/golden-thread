...
`<base>/../../templates/watch.md`; its frontmatter says what to watch (url, label, track,
branch, watch_paths, current_version, p0_when, gh_repo). A cron job (`gt_watch.py fetch`) asks
...
Since 0.15.0 watch is a Golden Thread module (plugin `gt-watch`, module `watch`), not part
of gt core: it is invoked as `/gt-watch:gt-watch <command>`, and `bash install.sh --without
watch` removes it — skill, script, hook and all.
...
3. If it says the `watch` setting is off, offer to switch it on
   (`python3 ~/.claude/golden-thread/hooks/gt_settings.py set watch report` — gt's
   settings tool; the module ships only `gt_watch.py`) and to install the
...
4. **On a P0**, file it in `<vault>/INBOX.md` as one line, under a vault claim like
   ...
   `- [ ] P0 upstream: <label> <ref> — <reason> (/gt-watch:gt-watch show <slug>)`
