...
## Version gt 0.12.7 / gt-wiki 0.1.2
...
## Core Rules (gt 0.12.7)

Golden Thread defines a tiered rule model that separates rules by scope and enforcement strength.
**Ten Core rules ship as of 0.12.7**, up from one at 0.6.0 (the count read "seven" from 0.9.10 through 0.12.3, one behind the files):
...
Installs both `gt` (v0.12.7) and `gt-wiki` (v0.1.2) as separate plugins under the `golden-thread-plugin` marketplace. Requires Python 3.8+.
