...
def install_core_rules(vault: Path, wire_hooks: bool = True, settings_path: Path = None,
    ...
    FALLBACK = [
        ("UserPromptSubmit", "inject_core_rules.sh"),
    ...
    try:
        ...
        pairs = [(r["event"], r["script"]) for r in mod.HOOK_REGISTRATIONS
                 if str(r.get("owner", "")).startswith("vault_init.py")]
        if not pairs:
            raise ValueError("no vault_init-owned registrations declared")
    except Exception:
        pairs = FALLBACK
    wanted = [(event, hooks_dir / script) for event, script in pairs]
...
def seed_vault_workspace(vault: Path):
