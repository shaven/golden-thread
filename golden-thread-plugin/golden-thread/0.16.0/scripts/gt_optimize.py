"""gt_optimize.py -- find content that costs context and earns nothing back. REPORTS ONLY.

Memory files are read into every session. A duplicated fact is paid for on every turn, in every
project, forever. This finds that waste. It does not remove it.

  gt_optimize.py --vault V [--project SLUG] [--json]
...
def dead_index_rows(vault, path, rel, lines):
    """A MEMORY.md row pointing at a file that is not there. Safe: it can never load."""
...
def oversized_global_memory(rel, lines):
    ...
    return [_finding("memory-bloat", JUDGEMENT, rel, 1,
                     ...
                     "Move the detail to Knowledge/ and leave the fact."
                     % (len(body), MEMORY_LINE_BUDGET))]
