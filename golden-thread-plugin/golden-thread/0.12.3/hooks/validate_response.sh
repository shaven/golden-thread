...
print(json.dumps({
    "decision": "block",
    "reason": (
        "Core rule violated (core_timestamp_every_message): the timestamp must precede "
        "ALL text you emit this turn — INCLUDING a short preamble before a tool call. "
        "This check reads the FIRST text block of the turn, not your final answer, so "
        "'Let me check X:' -> tool call -> timestamped summary FAILS. Put the timestamp "
        "at the very start of your first text, e.g. '2026-08-16 15:32 CDT — checking X:'. "
        "Use the value injected by the UserPromptSubmit hook; do not guess it. "
        "NOTE: re-sending only the final answer makes the user see the reply TWICE — "
        "fix the FIRST text block instead."
    ),
}))
sys.exit(ALLOW)
PY
