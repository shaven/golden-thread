...
description: "CORE rule — in every project, work that splits into independent units runs in parallel, within ONE machine-wide budget shared by every session (ADR-7). Serial execution on a multi-core machine is a choice that must be justified. Governs what the assistant RUNS and what it WRITES. Reminder tier: 'would parallelism have helped' is not mechanically checkable."
metadata:
  node_type: memory
  type: core
  level: core
  ...
  promoted: 2026-09-12
imperative: "Parallelise any work that can be parallelised, in every project — independent units run concurrently within one machine-wide budget shared by every session, and serial execution must be justified, not assumed."
...
run concurrently within one machine-wide budget shared by every session, and serial
execution must be justified, not assumed.**
...
### The budget belongs to the machine, not to the run (ADR-7)

`parallel_max` caps the **sum** of local work across every golden-thread session,
subagent and tool on the host — not each run separately. On 2026-09-14 one session's
builders each honoured the ceiling on their own and together held a 16-core machine at
load 24–31. Every run was within budget; the machine was not.

- **Count local compute** — test workers, sandbox installs, builds, host-local sweeps.
  Agents and subagents do not count: they wait on the model, not on a core.
- **Two pools**: CPU-bound work against `cpu_max`, I/O-bound work against `io_max`.
- **Take what is free, never fewer than 1.** A busy machine means a narrower run, not a
  blocked one.

**Until the shared slot pool (`gt_slots.py`) ships, do it by hand:** before choosing a
worker count, look at the load average and at gt test or build work already running
(yours, your subagents', other sessions'), and size the run to what is left. Never start
a second copy of a run that is still going. Say what you saw and what you chose.
...
independent work and the degree of parallelism — or name the reason there isn't one —
after checking what the rest of the machine is already running. When you write the tool,
give it a `--jobs` flag defaulting to the budget, and draw from the shared pool once it
exists.
