...
Record every move **at the moment the destination file is written**, with one command
...
4. Re-index `MEMORY.md` and log with `retire` or `relocate` — with `--event relocate`
