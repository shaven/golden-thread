...
**Generated** (never hand-edited; regenerate instead):
- `TASKS.md` — `gt_tasks.py`
- `log.md` — `gt_log.py merge`, from one spool file per session
- `decisions.md` — `gt_adr.py merge <project>`, from one file per ADR

These three are shared by every session, which is exactly why none of them is written
directly. A generated file cannot be half-overwritten by a concurrent writer.
