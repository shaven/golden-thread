...
Two placeholders, resolved by the tour runner before it runs a `do:` line:
...
- `<scripts>` — the installed plugin's scripts directory: `<base>/../../scripts`, where
  `<base>` is the gt-demo skill's base directory.
