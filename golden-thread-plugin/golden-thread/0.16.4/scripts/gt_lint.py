...
Usage:
  ...
  --json      findings as {"version":1,"vault","findings":[{kind,path,line,message}],"counts"}
  --runbooks  read-only: only report lines duplicated (identical or difflib ratio >= 0.9)
              ...
              than two runbooks prints "nothing to compare" and exits 0.
...
def check_adr_collision(vault: Path, findings: list, suppressed: set):
    ...
    for dec in sorted((vault / "Projects").glob("*/decisions.md")):
        ...
        for n in dupes:
            findings.append({
                "check": "adr-collision",
                "path": rel,
                "message": f"ADR-{n} is used by more than one decision in {dec.parent.name}",
...
def check_generated_hand_edited(vault: Path, findings: list, suppressed: set):
    ...
    for dec in sorted((vault / "Projects").glob("*/decisions.md")):
        targets.append((dec, "decisions", dec.parent.name))
...
def write_queue(vault: Path, findings: list, queue_path: Path):
    ...
    Hence, in order:
      ...
      2. compare the file against the digest of what we generated last time, and if it
         differs copy it aside BEFORE touching it and say so -- ticks come across, but
         anything else a person or gt-promote put in the file does not, and a backup
         nobody is told about is not a backup;
    ...
    An untouched queue takes neither a note nor a backup, so the warning keeps meaning.
    """
    notes = []
    ...
    if queue_path.exists() and (current is None or _sha(current) != recorded):
        stamp = dt.datetime.now().strftime("%Y-%m-%d-%H%M%S")
        bdir = vault / QUEUE_BACKUPS
...
def main():
    ...
    parser.add_argument("--runbooks", action="store_true",
                        help="Only detect content duplicated across Projects/**/runbook.md (read-only)")
