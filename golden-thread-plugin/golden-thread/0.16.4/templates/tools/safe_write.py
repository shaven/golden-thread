...
## Degradation is reported, never swallowed (2026-09-17)

Strategy 2 is NOT atomic: it truncates the target and writes into it, so a crash
or a full disk mid-write leaves the file short. Until now a failed (1) fell
...
So: strategy 2 now returns "direct-degraded", and the exception from (1) is
carried forward -- printed once to stderr before the fallback runs, and recorded
as `why_not_atomic` on the ledger entries for (3) and (4). The fallback itself
stays: on a filesystem where `os.replace` genuinely cannot work, a truncating
write is the only write there is. The defect was the silence, not the fallback.
...
def write(target, data, mode="w"):
    ...
    The strategy name is the truth about HOW the bytes landed, and the four names
    ...
    Nothing below strategy 1 is silent any more: the exception that defeated the
    atomic write is printed to stderr before the fallback is attempted, and is
    carried into the ledger entry as `why_not_atomic` for 3 and 4.
    ...
    try:
        ...
        _record({"target": target, "wrote_to": side, "strategy": "sidecar",
                 "action": "mv %s %s" % (side, target), "why_not_atomic": why})
    ...
    _record({"target": target, "wrote_to": fallback, "strategy": "ledger-dir",
             "action": "mv %s %s" % (fallback, target), "why_not_atomic": why})
