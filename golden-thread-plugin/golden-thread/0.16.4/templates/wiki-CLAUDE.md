...
Filing platform knowledge as a `Knowledge/` page (`/gt-wiki:gt-wiki-ingest`) and
refreshing a standalone wiki (`/gt-wiki:gt-wiki-refresh`) come from the optional **gt-wiki**
module. If the gt-wiki plugin is not installed (its skills are not listed), say so and
name `bash install.sh --with wiki` instead of invoking it.

The same holds for the other optional modules: `/gt-watch:gt-watch` (upstream repo
watch), the session report card (gt-report-card, no command of its own) and
`/gt-farm:gt-farm` (hand bulk work to an external AI as a packet; off by default). A
module that is not installed has no skills listed; name `bash install.sh --with <module>`.
