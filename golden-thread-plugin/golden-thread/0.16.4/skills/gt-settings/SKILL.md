---
name: gt-settings
description: "View and change what Golden Thread does on its own — component drift checking at session start, the session report card at compact, the upstream watch, and whatever an installed module adds. Every automatic behaviour is listed here and every one can be switched off."
---
...
| `report_card` (report-card module) | `off` · `minimal` · `full` | `minimal` |
