...
- Run `vault_init.py create-project --vault "<vault>"` with the gathered info
...
history (`- [x] the thought → [[slug]]`). In the same step, record the filing as an
event — one per line filed, naming where it went (the README for a task, `idea.md` for
a new project; `create-project` has already emitted `create` for the project itself):
```bash
python3 "<vault>/Projects/golden-thread/tools/gt_events.py" --vault "<vault>" emit \
  --kind file --item INBOX.md --from INBOX.md --to "Projects/<slug>/README.md" \
  --project <slug> --note "<the thought, first 100 chars>"
```
For a daily-note item use `--item`/`--from` with the daily note's vault-relative path.
A refused event prints one stderr line and changes nothing else; fix it and re-run. Never delete the line; the user prunes
