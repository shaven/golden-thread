...
SETTINGS = {
    ...
    "parallel_max": {
        ...
        "detail": (
            "auto  as many as possible -- cores for CPU-bound work, and generously more\n"
            "      than cores for I/O-bound work, which is what the tools here do  (default)\n"
            "N     never more than N workers at once, whatever the work\n"
            "\n"
            "`auto` is deliberately not a number. A stored count is wrong on the next\n"
            "machine and wrong again after a hardware change, and this config file syncs\n"
            "between machines -- the same argument as computing task priority at the moment\n"
            "of asking rather than storing it. The tool decides the degree from the shape of\n"
            "the work; this setting only caps it.\n"
            "\n"
...
def parallel_jobs(want, io_bound=False):
    ...
    cap = min(cores + 4, 20) if io_bound else cores
