...
```bash
python3 <vault>/Projects/golden-thread/tools/gt_adr.py allocate <project> --title "<title>"
```

Write the body into the slot it names, then `gt_adr.py merge <project>`. Never pick a
number by reading `decisions.md` and adding one.
