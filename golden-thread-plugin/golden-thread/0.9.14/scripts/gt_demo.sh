...
# Commands: start | end | clean | remove | status
#
...
clean)
  ...
  echo "Reopen the demo session:  cd \"$DEMO\" && GT_VAULT=\"$DEMO\" claude"
  ;;
...
*)
  echo "Usage: gt_demo.sh <start|end|clean|remove|status>"
  exit 1
  ;;
