...
HOOK_REGISTRATIONS = (
    ...
    {"event": "SessionEnd", "script": "gt_report_card.py",
     "args": [], "owner": "install.sh"},
    # PreCompact and SessionEnd output reaches neither the model nor the user (Claude
    # Code hooks reference, verified 2026-09-13), so the card and its close-out question
    # were never seen. Those two runs now leave a pending notice; this SessionStart run
    # surfaces it, where systemMessage reaches the user and stdout reaches the model (0.12.9).
    {"event": "SessionStart", "script": "gt_report_card.py",
     "args": ["surface", "--hook"], "owner": "install.sh"},
