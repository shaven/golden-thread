...
def report():
    ...
    if not pend:
        if watches:
            ...
            print("GOLDEN THREAD watch: on, but no watches yet — /gt:gt-watch add <git-url>.")
        return 0
    ...
    print("  explain: /gt:gt-watch show <slug>   mark seen: /gt:gt-watch ack [<slug>]")
    return len(p0)
...
def hook():
    ...
    try:
        import gt_settings
        r, text = gt_settings.capture(report)
...
def _template():
    for p in (os.path.join(os.path.dirname(HERE), "templates", "watch.md"),
              os.path.expanduser("~/.claude/plugins/marketplaces/golden-thread-plugin/plugins/gt/templates/watch.md")):
        if os.path.isfile(p):
