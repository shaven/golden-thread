...
class Ctx:
    ...
    def __init__(self, home, release_dir, release, dry_run, mid=None):
        self.home = Path(home)
...
def apply_install_choices(ctx):
    ...
    ctx.write_json(INSTALL_CHOICES, out)
...
def plan(home, release_dir, release, dry_run):
    """-> (state, [(migration, reapply)]) for everything pending, in registry order."""
    state = read_state(home)
    todo = []
    for m in registry():
        ctx = Ctx(home, release_dir, release, dry_run, m.id)
        try:
...
def cmd_status(a, release):
    _state, todo = plan(a.home, a.release, release, True)
    if not todo:
...
def cmd_run(a, release):
    state, todo = plan(a.home, a.release, release, a.dry_run)
    if a.dry_run:
        if not todo:
            print("Machine migrations: none pending")
        for m, reapply in todo:
        ...
        return 0
    for m, reapply in todo:
        ctx = Ctx(a.home, a.release, release, False, m.id)
        try:
            m.apply(ctx)
            if m.needed(ctx):
