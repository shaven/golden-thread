...
do: Invoke the gt-watch skill to add a watch on `file://$GT_VAULT/.demo/upstream/widget-lib.git` labelled "Widget library". Then run `bash ~/.claude/plugins/marketplaces/golden-thread-plugin/plugins/gt/scripts/gt_demo.sh upstream-release` to publish widget-lib v1.0.1, run `python3 ~/.claude/plugins/marketplaces/golden-thread-plugin/plugins/gt/scripts/gt_watch.py fetch` (the job cron runs hourly), and finally `python3 ~/.claude/plugins/marketplaces/golden-thread-plugin/plugins/gt/scripts/gt_watch.py --hook` to show the P0 exactly as the next session start will report it.
...
## Act 11 — Inbox, close, and the receipt
