...
gt has ever shipped (dev/shipped_hashes.py):
  ...
  anything else                     -> the owner's, and KEPT -- a tool and a hook alike.
                                       Reported either way, and the report says whether
                                       the change is committed, because an uncommitted one
                                       exists nowhere else.

That last rule is flatter than it was through 0.16.4, where this paragraph said a COMMITTED
hook was "backed up outside the vault, then replaced". No code ever did that: `refresh_githooks`
returns without writing in both branches, and the inline comment there says so. The docstring
was describing a plan from before 0.15.0 (corrected 2026-09-18). Replacing an owner's committed
hook would also be the 0.15.0 defect this file was extracted to fix, so the CODE is right.
`vault_refresh.py backup` still captures every file an install may write, hook or not, before
it writes -- that, not a per-hook copy, is where the safety net is.
