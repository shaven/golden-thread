...
`--force` replaces an existing handoff file. Without it an existing file is REFUSED rather than
overwritten: a handoff is someone's record of a session, and overwriting one loses it silently.
...
WRITTEN, for any of four reasons, each of which prints its own line on stderr: the project could
not be read, the destination resolves outside the vault, the destination exists and --force was
not given, or the write failed. They share a code deliberately -- to a caller they are one
