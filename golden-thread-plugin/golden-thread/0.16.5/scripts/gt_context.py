...
def main(argv=None):
    ...
    if args.json:
        ...
        if sections:
            ...
            print("no model-reachable definitions are in effect", file=sys.stderr)
    # "Nothing to render" is decided for BOTH surfaces here. It used to sit inside the
    # non-JSON branch, so `--json` with nothing in effect exited 0 -- "rendered" -- having
    # rendered nothing, and a caller keying on the exit code could not tell an empty vault
    # from a populated one (found 2026-09-18).
    if not sections:
