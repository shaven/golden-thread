...
# CHARACTERS, not bytes: this caps a str, and a multi-byte character counts once. The name and
# the truncation note both said "bytes", which overstated the cap by up to 4x for non-ASCII
# output (found 2026-09-18). The number is a flood stop, so the unit only has to be honest.
...
def sanitise(text, member, cap=MAX_OUTPUT_CHARS):
    ...
    if truncated:
        out.append("  [%s] ... output truncated at %d characters" % (member, cap))
    return "\n".join(out)
