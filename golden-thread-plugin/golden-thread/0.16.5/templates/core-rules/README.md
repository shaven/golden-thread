...
| `core_concurrent_session_claim.md` | **Core/Validated** — one writer per file. Register the session, claim before writing, never write a file another live session holds. Enforced by the `PreToolUse` guard. |
...
| `core_parallel_when_beneficial.md` | Core/Reminder — parallelise divisible work in every project, within one machine-wide `parallel_max` budget shared by every session (ADR-7, 2026-09-14); serial execution must be justified. Governs what is run AND what is written. Designated by the user 2026-09-12. |
