...
# git commit -v: git discards this line and everything below it (the diff).
_SCISSORS = re.compile(r"^\S -+ >8 -+$", re.M)
...
def _repo_root(git_dir):
    """The working tree this git dir belongs to, or None when there is none.

    `os.path.dirname(git_dir)` is right only for the ordinary `<root>/.git` layout. A linked
    worktree's git dir is `<root>/.git/worktrees/<name>` and a submodule's is
    `<root>/.git/modules/<name>` -- the basename is never `.git` there, so this returned None,
    `rel` stayed an absolute path and `_task_for` gave up: attribution silently degraded in
    exactly the two layouts _git_dir's comment above says are handled (fixed 2026-09-18).

    So ask git, which knows, and keep the dirname rule as the fallback for a git dir git
    cannot answer for (a bare repo has no working tree at all, and None is correct there)."""
    try:
        out = subprocess.run(["git", "--git-dir", git_dir, "rev-parse", "--show-toplevel"],
                             capture_output=True, text=True, timeout=5)
...
def apply_to_message(msg_path, git_dir, staged=None):
    ...
    if not lines:
        return 0
    # Trailers go ABOVE a scissors line: anything below it is cut, and post-commit
    # would then drain the ledger with the attribution never recorded.
    cut = _SCISSORS.search(body)
    body, tail = (body[:cut.start()], body[cut.start():]) if cut else (body, "")
...
def main():
    ...
    if cmd == "show":
        ...
        if len(sys.argv) < 3:
            ...
            return 2
        # Always restricted, even to nothing: an empty commit contains no file,
        # so it must claim none of the pending edits.
        print(apply_to_message(sys.argv[2], g, _staged(g)))
    elif cmd == "clear":
        # Keep anything not in this commit: an edit made but not yet committed
        # must stay attributed for whenever it IS committed.
        staged = _staged(g)
        n = clear(g, keep_unstaged={r.get("path") for r in load(g)} - staged)
        print("cleared %d entr(ies)" % n)
    elif cmd == "commit-clear":
        # post-commit: the just-committed paths are gone from the index, so
        # anything still modified in the tree is what must be retained -- and so is
        # a new file left out of the commit, which `git diff` cannot see (untracked).
