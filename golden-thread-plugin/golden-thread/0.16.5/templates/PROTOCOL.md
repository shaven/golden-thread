...
1. **Register.** Write `Projects/golden-thread/sessions/<session-id>_<date>_<time>.md`
   when you begin substantive work: what you are doing, and one `` - `path` `` bullet
   per file you have open. (There is no `files_claimed` field — earlier revisions of
   this document named one and no code ever wrote or read it.)
...
4. **If a file is claimed, do not edit it.** Stage your change under
   ...
   **`log.md` and `decisions.md` no longer need this**: they are generated from
   per-session spool files, so nothing is shared to collide over. Use
   `tools/gt_log.py add` and `tools/gt_adr.py allocate`.
...
**Re-registering keeps what you hold.** `register` on an id you already hold — and
`--resume` — carries the existing claims forward and unions `--files` into them. Until
0.16.5 it rebuilt the list from `--files` alone, so re-registering mid-session silently
dropped every claim made since, which disarmed this whole convention without saying so.

**One id can have two registrations, and then you must aim.** `--new` deliberately
creates a second entry for the same id (a subagent wanting its own). When more than one
exists, `claim`, `beat` and `release` **refuse rather than guess** and print both names:
pass `--entry <file>` to name the one you mean. Before 0.16.5 they silently picked by
sort order, which meant a process could claim into another's registration and a `release`
could clear claims it did not own.

**`release` will not delete someone else's.** It refuses a registration whose pid is
another running process (`--force` overrides), and reports how many claims it cleared.
...
- **`log.md` is GENERATED** from `Projects/golden-thread/spool/log/`. Never hand-edit
  it and never `git checkout` it: add with `tools/gt_log.py add`, which writes only
  your own session's file. To remove your own line, delete it from your spool file and
  re-merge — no other session's work is in that file.
- **`decisions.md` is GENERATED** from `spool/decisions/<project>/`. Allocate a number
  with `tools/gt_adr.py allocate <project>` before writing an ADR; never read the file
  and add one.
