...
- **new project ideas** → run `vault_init.py create-project --vault "<vault>" --name "<slug>"` for each new project slug
...
item run `gt_events.py --vault "<vault>" merge` once. New project scaffolds emit their
own `create` event.
