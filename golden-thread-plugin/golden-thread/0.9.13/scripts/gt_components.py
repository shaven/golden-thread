...
# repair advice differs. install.sh wires the six reporting hooks; vault_init.py
# wires the three enforcement hooks when a vault is initialised. Declaring all
# nine here means the check covers the enforcement layer too -- gt_lint.py checks
...
def main():
    ...
    if not args:
        ...
        return 2
    cmd = args[0]
    # Skip flags when locating the positional version dir, or `wiring --owner X <dir>`
    # would take "--owner" as the directory and report a bogus missing manifest.
    ...
    if cmd in ("manifest", "check", "apply", "hook-registrations", "wiring") and not vdir:
