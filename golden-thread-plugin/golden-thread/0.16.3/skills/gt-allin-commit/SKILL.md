...
past the one mechanism enforcing Core rule 4, so the same check is made here, directly, against
the same ledger. Protection cannot depend on who is holding the pen.
...
- **Say which tests the receipt covers** when reporting a successful commit — Core rules 4 and
  10 together mean naming the evidence, not just asserting it.
