...
def emit(text, event="SessionStart"):
    ...
    When stdout is a terminal the caller is a human running the command by hand, so
    the plain text is printed unchanged and no JSON leaks into their shell.
    """
    text = (text or "").strip()
    if not text:
        return
