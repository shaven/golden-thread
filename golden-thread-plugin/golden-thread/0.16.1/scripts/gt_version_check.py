...
def plugins_for(src_root):
    """PLUGINS, then every other module in the source tree, as (key, subdir, label, module).

    PLUGINS named gt and gt-wiki only, so the demo (0.14.0) and the watch, report-card and
    farm modules (0.15.0) were never version-checked: a stale gt-watch cache beside a newer
    source release said nothing. A module is a top-level dir whose newest release carries
    module.json; its plugin and module names are read from there. Unreadable -> skipped.
    """
    out, seen = list(PLUGINS), {p[1] for p in PLUGINS}
    try:
        dirs = sorted(os.listdir(src_root))
    except Exception:
        return out
    for d in dirs:
        if d in seen or d.startswith("."):
            continue
        vers = available(os.path.join(src_root, d))
        if not vers:
            continue
        try:
            with open(os.path.join(src_root, d, vers[-1], "module.json")) as fh:
                m = json.load(fh)
            plugin, name = m["plugin"], m["name"]
        except Exception:
            continue
        if isinstance(plugin, str) and isinstance(name, str):
            out.append(("%s@golden-thread-plugin" % plugin, d, plugin, name))
    return out
...
def report(src_root, pol=None):
    ...
    for key, subdir, label, module in plugins_for(src_root):
