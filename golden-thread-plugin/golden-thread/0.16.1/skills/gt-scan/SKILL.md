...
Everything the scan looks for comes from **packs**, not from the scripts — see `/gt:gt-registry`
and `SUBMISSIONS.md`. A contributed language pack changes what the scan does with no code change.
