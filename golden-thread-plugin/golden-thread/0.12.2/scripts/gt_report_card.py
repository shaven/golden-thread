...
def main():
    ...
    cands = closeout_candidates(v) if _registry_get("closeout_check", "ask") != "off" else []
    ...
    if fe:
        ...
        for f in fe:
            print("   - %s" % f)
    if cands:
        print("  Projects that look finished -- ASK THE USER whether to close each one:")
        for c in cands:
            print("   - %s (%s): %s" % (c["slug"], c["stage"], "; ".join(c["reasons"])))
        print("    Record the answer: python3 Projects/golden-thread/tools/gt_closeout.py "
              "answer <slug> yes|no|later \"<why>\"")
        print("    Closing means: stage: complete in README.md, leftover tasks to p:: 7, then gt-work.")
    return 0
