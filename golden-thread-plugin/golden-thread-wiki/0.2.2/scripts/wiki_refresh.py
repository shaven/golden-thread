...
Exit: 0 nothing changed, 1 changes found, 2 usage/error.
  A source whose check failed (state `error`) makes the exit 2 - never 0,
  which would be a false clean - unless changes were also found: 1 wins,
  because the changed sources still need acting on and the report lists
  the errors beside them.
"""
...
def as_list(v):
    # frontmatter holds a list, or a scalar for a single value (`supersedes: Sources/x.md`)
    return v if isinstance(v, list) else [v] if v else []

def usage_error(msg):
    print(msg, file=sys.stderr)
    sys.exit(2)  # sys.exit(<message>) would exit 1, which means changes found
...
def resolve(vault, pages, sources, a):
    ...
    for s in sources.values():
        for old in as_list(s["fm"].get("supersedes")):
    ...
    if a.all:
        ...
        if not pg:
            usage_error(f"no Knowledge page matching {a.page!r}")
        want = {norm(x) for x in as_list(pg["fm"].get("sources"))}
        ...
        usage_error("scope required: --page, --repo, --topic, or --all")
...
def check_local(src, fetched, a):
    ...
    # git reports the resolved toplevel, so a symlinked `local:` must be resolved too
    rel = os.path.relpath(os.path.realpath(local.rstrip("/")), repo)
...
def main():
    ...
    for src in resolve(a.vault, pages, sources, a):
        ...
        r["cited_by"] = sorted(p["file"] for p in pages.values()
                               if norm(src["file"]) in {norm(x) for x in as_list(p["fm"].get("sources"))})
        results.append(r)
    ...
    sys.exit(1 if changed else 2 if errors else 0)
