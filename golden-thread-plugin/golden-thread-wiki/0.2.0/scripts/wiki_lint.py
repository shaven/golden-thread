...
def frontmatter(text):
    ...
    for line in raw.splitlines():
        ...
        if m and not line.startswith((" ", "\t")):
            key = m.group(1)
            val = m.group(2).strip()
            # 2026-09-08: an inline YAML/JSON list -- `sources: ["a", "Sources/b.md"]` or `[]` --
            # used to be kept as ONE string, so every page that declared its sources inline
            # was reported as citing a missing source named after the whole bracket.
            if val.startswith("[") and val.endswith("]"):
                ...
                if inner:
                    try:
                        ...
                        items = [str(x) for x in _json.loads(val)]
                    except Exception:
    ...
    for d, store in ((kdir, pages), (sdir, sources)):
        ...
        for fn in os.listdir(d):
            if not fn.endswith(".md"):
                continue
            if store is pages and fn == "_template.md":
                continue  # the page template vault_init seeds is not a page
...
def main():
    ...
    # 2026-09-08: inside a Golden Thread vault the wiki links out to projects, memory notes and
    # root pages (INFRASTRUCTURE, PROTOCOL, CONVENTIONS). Those are real targets and real sources.
    # Register every .md in the vault by stem so links and citations to them resolve; the orphan
    # and index logic still applies to Knowledge/ pages only.
    vault_slugs = set()
    for d, dirs, files in os.walk(vault):
        dirs[:] = [x for x in dirs if not x.startswith(".") and x != "node_modules"]
        for fn in files:
            if fn.endswith(".md"):
                vault_slugs.add(norm(fn))
    # a project folder is addressable by its slug too ([[shome-security]])
    projects = os.path.join(vault, "Projects")
    if os.path.isdir(projects):
        for d, dirs, files in os.walk(projects):
            if "README.md" in files and d != projects:
                vault_slugs.add(os.path.basename(d).lower())
    all_slugs |= vault_slugs
    ...
    for slug, p in pages.items():
        ...
        if not src_list:
            ...
            for s in src_list:
                if norm(s) not in sources and norm(s) not in vault_slugs:
