...
rewritten; the report shows both paths so a human can decide). A config
holding other settings but no vault_path gains vault_path and keeps its keys.
"""
...
def seed_template(template_name, dest, results, substitutions=None):
    ...
    for old, new in (substitutions or {}).items():
        if new is not None:  # None = no value given; the placeholder stays and is reported
...
def write_config(config_path, vault_path, results):
    ...
    if os.path.exists(config_path):
        ...
        # compared resolved, the way gt's vault_init records it: the same vault reached
        # through a symlink (or macOS /var -> /private/var) is not a second vault
        if current and os.path.realpath(os.path.expanduser(current)) == os.path.realpath(vault_path):
            report_line(results, "skipped", config_path, "config already points here")
            return True
        if not current:
            # other settings only (e.g. install_demo): no vault to conflict with - merge
            existing["vault_path"] = vault_path
            with open(config_path, "w", encoding="utf-8") as f:
                json.dump(existing, f, indent=2)
                f.write("\n")
            report_line(results, "updated", config_path, f"vault_path -> {vault_path} (other keys kept)")
            return True
        report_line(results, "conflict", config_path,
...
def init_wiki(vault, domain, results):
    ...
    subs = {"<YOUR DOMAIN HERE>": domain or None}
