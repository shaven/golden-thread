...
vault path in `~/.claude/vault-config.json`. (If `$GT_VAULT` is set the session is pinned to another vault — say so and stop rather than reconfigure.) It is idempotent (existing
