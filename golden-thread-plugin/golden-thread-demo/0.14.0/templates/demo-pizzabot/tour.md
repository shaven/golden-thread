...
module's `demo/act.md`), ordered by module name and numbered on after these. Install a
module and its act appears in the next tour; remove it and the act is gone. An act that
belongs to a module lives in that module, never here.
...
Three placeholders, resolved by the tour runner before it runs a `do:` line:
